...
<body class="bg-white text-gray-900 dark:bg-gray-900 dark:text-white transition min-h-screen" class="min-h-screen bg-white text-gray-900 dark:bg-gray-900 dark:text-white transition" class="transition min-h-screen bg-white text-gray-900 dark:bg-gray-900 dark:text-white">
  ...
  <main class="px-4">
    ...
    <input type="text" id="recherche" placeholder="Rechercher une tâche..." class="w-full p-2 mb-4 border border-gray-300 rounded dark:bg-gray-800 dark:border-gray-600 dark:text-white" />
    ...
    <ul class="taches space-y-3">
      <?php foreach ($taches as $tache): ?>
        ...
        ?>
        <li class="tache flex items-center justify-between bg-white dark:bg-gray-800 p-3 rounded shadow border-l-4 <?= $classePriorite ?> <?= $tache->estTerminee() ? 'opacity-60' : '' ?>" data-priorite="<?= $prio ?>">
          <div class="flex items-center gap-4 w-full">
            <span class="text-xs font-semibold <?= $couleurPriorite ?> min-w-[90px]"><?= $textePriorite ?></span>
            <input type="checkbox" class="checkbox-terminee w-5 h-5 accent-green-600" data-id="<?= $tache->getId() ?>" <?= $tache->estTerminee() ? 'checked' : '' ?>>
            <span class="texte flex-grow break-words <?= $tache->estTerminee() ? 'line-through text-gray-500' : 'text-gray-900 dark:text-white' ?>">
